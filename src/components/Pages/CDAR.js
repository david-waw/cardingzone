...
import './shop.scss'
import SideNav from "../todayDeal/NavBar"
import ReactPaginate from "react-paginate"
import { useHistory } from "react-router-dom";
import CustomButton from '../custombutton/custombutton'


const   CDAustraliaRandom= () => {
        ...
        <>
            ...
            <div className='productsContainer'>
                {product.length === 0 && <div>slow internet...no products to display</div>}
                        {product.filter(name => name.category.includes('CreditDebit'))
                            .filter(name => name.country.includes('australia'))
                            .filter(Prodname => Prodname.ProductName.includes('Random'))
...
export default  CDAustraliaRandom
